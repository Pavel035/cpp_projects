...
void MyString::write(std::ofstream& os) const
{
	os.write((const char*)&_length, sizeof(size_t));
	os.write(c_str(), _length*sizeof(char));
}

void MyString::read(std::ifstream& is)
{
	size_t length;
	is.read((char*)&length, sizeof(size_t));
	char* buffer = new char[length + 1];
	is.read(buffer, length*sizeof(char));
	buffer[length] = '\0';
